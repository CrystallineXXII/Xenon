...
class App:
    ...
    def add_button(
        ...
        *,
        ...
        self.widgets.append(
            Button(text, pos, color, bgColor, outlineColor, font, size, action)
        )
        self.widgets[-1].id = self.widget_count
        self.widget_count += 1
        return self.widgets[-1]
    ...
    def add_textbox(
        ...
        *,
        ...
        self.widgets.append(
            TextBox(
                text, size, pos, color, activeColor, bgColor, outlineColor, font, fsize
            )
        )
        self.widgets[-1].id = self.widget_count
        self.widget_count += 1
                        ...
                        if isinstance(widget, TextBox):
                            if widget.active:
                                if event.key == pg.K_BACKSPACE:
                                    ...
                                    print(widget.link)
                                    if widget.link != None:
                                        if isinstance(widget.link, TextBox):
                                            widget.link.active = True
                                        elif isinstance(widget.link, Button):
                                            widget.link.action()
                                        break
                                else:
...
if __name__ == "__main__":
    print(
        "Xenon.py is not meant to be run as a standalone program. Please import it into your program."
    )
    sys.exit()
